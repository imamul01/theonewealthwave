  ...
  try {
    ...
    const loadRewardRank = async () => {
      try {
        ...
        if (docSnap.exists) {
          ...
          // Check if we need to migrate from old structure to new 7-level structure
          if (state.rewardList.length > 0 && !state.rewardList[0].level1Business) {
            // Migrate old structure to new 7-level structure
            state.rewardList = state.rewardList.map((item, index) => ({
              rank: index + 1,
              totalBusiness: item.totalBusiness || (index + 1) * 1000,
              level1Business: item.powerLeg || (index + 1) * 100,
              level2Business: item.otherLegs || (index + 1) * 100,
              level3Business: (index + 1) * 100,
              level4Business: (index + 1) * 100,
              level5Business: (index + 1) * 100,
              level6Business: (index + 1) * 100,
              level7Business: (index + 1) * 100,
            ...
            showMessage('Migrated to 7-level reward structure!', 'success');
          }
        }
        if (!state.rewardList.length) {
          // Create 7 levels with new structure
          state.rewardList = Array.from({ length: 7 }, (_, i) => ({
            rank: i + 1,
            totalBusiness: (i + 1) * 1000,
            level1Business: (i + 1) * 100,
            level2Business: (i + 1) * 100,
            level3Business: (i + 1) * 100,
            level4Business: (i + 1) * 100,
            level5Business: (i + 1) * 100,
            level6Business: (i + 1) * 100,
            level7Business: (i + 1) * 100,
          ...
          state.rewardList.forEach(item => {
            tbody.appendChild(createElement('tr', {},
              ...
              createEditableNumberCell(item.totalBusiness, v => {
                ...
                // Validate that total business is at least the sum of all 7 levels
                const totalLevelBusiness = (item.level1Business || 0) + (item.level2Business || 0) + 
                                         (item.level3Business || 0) + (item.level4Business || 0) + 
                                         (item.level5Business || 0) + (item.level6Business || 0) + 
                                         (item.level7Business || 0);
                if (item.totalBusiness < totalLevelBusiness) {
                  showMessage('Total Business must be >= Sum of all 7 levels', 'error');
                  item.totalBusiness = totalLevelBusiness;
                }
              }, 0),
              createEditableNumberCell(item.level1Business || 0, v => item.level1Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.level2Business || 0, v => item.level2Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.level3Business || 0, v => item.level3Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.level4Business || 0, v => item.level4Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.level5Business || 0, v => item.level5Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.level6Business || 0, v => item.level6Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.level7Business || 0, v => item.level7Business = Math.max(v, 0), 0),
              createEditableNumberCell(item.rewardIncome, v => {
                ...
                const totalLevelBusiness = (item.level1Business || 0) + (item.level2Business || 0) + 
                                         ...
                                         (item.level7Business || 0);
                if (item.totalBusiness < totalLevelBusiness) {
                  showMessage('Cannot set reward: Total Business must be >= Sum of all 7 levels', 'error');
                  item.rewardIncome = 0;
                }
              }, 0),
              createElement('td', {},
    ...
    const assignRewardsToUsers = async () => {
      try {
        ...
        for (const userDoc of usersSnap.docs) {
          ...
          // Calculate business for each of the 7 levels
          let level1Business = 0, level2Business = 0, level3Business = 0, level4Business = 0;
          let level5Business = 0, level6Business = 0, level7Business = 0;
          
          // Level 1 (direct referrals)
          if (teamData[0]) {
            level1Business = teamData[0].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Level 2
          if (teamData[1]) {
            level2Business = teamData[1].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Level 3
          if (teamData[2]) {
            level3Business = teamData[2].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Level 4
          if (teamData[3]) {
            level4Business = teamData[3].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Level 5
          if (teamData[4]) {
            level5Business = teamData[4].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Level 6
          if (teamData[5]) {
            level6Business = teamData[5].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Level 7
          if (teamData[6]) {
            level7Business = teamData[6].reduce((sum, member) => sum + (member.selfDeposit || 0), 0);
          }
          
          // Find eligible rank based on 7-level business
          const eligibleRank = state.rewardList.find(rank =>
            user.totalBusiness >= rank.totalBusiness &&
            level1Business >= rank.level1Business &&
            level2Business >= rank.level2Business &&
            level3Business >= rank.level3Business &&
            level4Business >= rank.level4Business &&
            level5Business >= rank.level5Business &&
            level6Business >= rank.level6Business &&
            level7Business >= rank.level7Business
          );
          ...
          if (eligibleRank) {
            ...
            // Add notification
            await db.collection('notifications').add({
              message: `Congratulations! You achieved Rank ${eligibleRank.rank} with a reward of ${formatCurrency(totalReward)} based on your 7-level team performance.`,
              user: user.email || user.id,
              createdAt: firebase.firestore.FieldValue.serverTimestamp()
            });
            ...
            await db.collection('income').add({
              ...
              status: 'credited',
              createdAt: firebase.firestore.FieldValue.serverTimestamp(),
              description: `Rank ${eligibleRank.rank} Reward - 7 Level Achievement`,
              rank: eligibleRank.rank,
              level1Business: level1Business,
              level2Business: level2Business,
              level3Business: level3Business,
        ...
        showMessage('7-Level rewards assigned to eligible users!', 'success');
      } catch (e) {
        console.error('Error assigning 7-level rewards:', e);
        showMessage('Failed to assign 7-level rewards', 'error');
      }
    };
    ...
    const addRewardLevel = () => {
      ...
      const newLevel = {
        ...
        level1Business: newRank * 100,
        level2Business: newRank * 100,
        level3Business: newRank * 100,
        level4Business: newRank * 100,
        level5Business: newRank * 100,
        level6Business: newRank * 100,
        level7Business: newRank * 100,
        rewardIncome: newRank * 100
      };
      state.rewardList.push(newLevel);
      renderRewardRank();
      showMessage(`Added new reward level ${newRank}!`, 'success');
    };
    ...
    const resetRewardLevels = async () => {
      if (confirm('Are you sure you want to reset all reward levels to default? This will remove all custom settings.')) {
        try {
          // Create 7 levels with default structure
          state.rewardList = Array.from({ length: 7 }, (_, i) => ({
            rank: i + 1,
            totalBusiness: (i + 1) * 1000,
            level1Business: (i + 1) * 100,
